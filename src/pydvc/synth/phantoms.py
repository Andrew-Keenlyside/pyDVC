...
def make_case(
    ...
    *,
    ...
    starting_point: tuple[float, float, float] | None = None,
) -> Path:
    ...
    ``points_xyz`` (and optionally ``point_id``) replaces the lattice with given
    points, e.g. a CCPi ``.roi``, kept as they are even if some subvolumes
    leave the volume. ``starting_point`` is written to the config's seeding.
    ...
    if points_xyz is None:
        ...
        point_id = np.arange(1, len(xyz) + 1, dtype=np.int64) if point_id is None else np.asarray(point_id, dtype=np.int64)
    ...
    cfg = RunConfig(
        ...
        seeding=SeedingSpec(strategy="wavefront",
                            start_point=tuple(float(v) for v in starting_point) if starting_point is not None else None),
