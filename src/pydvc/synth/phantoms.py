...
def _write_volume(path: Path, shape: tuple[int, int, int], dtype: np.dtype, block_fn, *, chunk: int, shard: int) -> None:
    """Generate a volume one write block at a time and store it one whole shard at a time."""
    ...
    for sbox in iter_blocks(shape, shard_shape):
        ...
        for bbox in iter_blocks(sbox.shape, (_WRITE_BLOCK,) * 3):
            box = Box(tuple(a + b for a, b in zip(sbox.lo, bbox.lo)), tuple(a + b for a, b in zip(sbox.lo, bbox.hi)))
            buf[bbox.slices()] = block_fn(box)
...
def make_case(
    ...
    *,
    ...
    """Write ``ref.ome.zarr``, ``def.ome.zarr``, ``points.roi``, ``truth.npz`` and ``config.yaml``; return the config path.
    ...
    The search points go in a CCPi ``.roi``; M3 adds the zarr-vectors store.
    """
    ...
    def noise(box: Box, stream: int) -> np.ndarray:
        if noise_sigma <= 0:
            return 0.0
        return _block_noise(box, seed, stream) * (noise_sigma / _CONTRAST)

    def ref_block(box: Box) -> np.ndarray:
        return _to_dtype(speckle_field(box, feature_size=feature_size, seed=seed) + noise(box, 1), dt)

    def def_block(box: Box) -> np.ndarray:
        return _to_dtype(_resample(field, box, feature_size=feature_size, seed=seed) + noise(box, 2), dt)

    _write_volume(out / "ref.ome.zarr", shape, dt, ref_block, chunk=chunk, shard=shard)
    _write_volume(out / "def.ome.zarr", shape, dt, def_block, chunk=chunk, shard=shard)
